...
> 目标读者：未来的面试官 / 评审者；目标效果：让人觉得"这个人确实懂评测，手上有真东西"。

---

## 1. 项目故事（为什么做这个项目）

作者是一名数据分析师，日常大量使用 AI 辅助写 SQL，发现它经常"悄悄出错"：join 错表、日期口径不对、聚合漏条件、把不存在的字段编出来——错了还不报错。于是系统性回答一个问题：

> **一个数据分析 Agent，到底能不能信？什么时候能信？错了怎么发现、怎么量化、怎么防止回归？**

为此没有只写几个测试脚本，而是实现了一个具备主流评测平台（LangSmith / Langfuse / Braintrust / DeepEval）核心能力的评测框架：
